...
! set MPI communicator
      ...
      if ( use_ddpdd_sum ) then
         ...
         call t_startf('shr:reprosum_ddpdd')

         call shr_reprosum_ddpdd(arr, arr_gsum, nsummands, dsummands, &
                              nflds, mpi_comm)
         repro_sum_fast = 1

         call t_stopf('shr:reprosum_ddpdd')

      else

         call t_startf('shr:reprosum_int')
...
! algorithm
               call t_startf("shr:repro_sum_allr_max")
               ...
               call t_stopf("shr:repro_sum_allr_max")
...
!$omp private(ithread, ifld, isum, arr_exp, arr_exp_tlmin, arr_exp_tlmax)
            do ithread=1,omp_nthreads
               call t_startf('shr:repro_sum_loopa')
               do ifld=1,nflds
               ...
               call t_stopf('shr:repro_sum_loopa')
            end do
...
! compare fixed and floating point results
      if ( present(rel_diff) ) then
         if (shr_reprosum_reldiffmax >= 0.0_r8) then
            ...
            call t_barrierf('shr:sync_nonrepro_sum',mpi_comm)
            call t_startf('shr:nonrepro_sum')
! record statistic
            nonrepro_sum = 1
...
!$omp         arr_shift, ilevel, i8_arr_level, arr_remainder, RX_8, IX_8)
      do ithread=1,omp_nthreads
       call t_startf('shr:repro_sum_loopb')
       do ifld=1,nflds
          ioffset = offset(ifld)
...
! over threads and processes do not overflow for ilevel==1.
          ...
          enddo
       enddo
       call t_stopf('shr:repro_sum_loopb')
      enddo
...
#if ( defined noI8 )
     ! Workaround for when shr_kind_i8 is not supported.
      call t_startf("shr:repro_sum_allr_i4")
      call mpi_allreduce (i8_arr_lsum_level, i8_arr_gsum_level, & 
                          veclth, MPI_INTEGER, MPI_SUM, mpi_comm, ierr)
      call t_stopf("shr:repro_sum_allr_i4")
#else
      call t_startf("shr:repro_sum_allr_i8")
      call mpi_allreduce (i8_arr_lsum_level, i8_arr_gsum_level, & 
                          veclth, MPI_INTEGER8, MPI_SUM, mpi_comm, ierr)
      call t_stopf("shr:repro_sum_allr_i8")
#endif
