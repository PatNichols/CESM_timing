...
subroutine shr_dmodel_readLBUB(stream,pio_subsystem,pio_iotype,pio_iodesc,mDate,mSec,mpicom,gsMap, &
  ...
  character(*), parameter :: subname = '(shr:dmodel_readLBUB) '
  character(*), parameter :: F00   = "('(shr:dmodel_readLBUB) ',8a)"
  character(*), parameter :: F01   = "('(shr:dmodel_readLBUB) ',a,5i8)"
  character(*), parameter :: F02   = "('(shr:dmodel_readLBUB) ',3a,i8)"

!-------------------------------------------------------------------------------
! PURPOSE:  Read LB and UB stream data
!----------------------------------------------------------------------------

  lstr = 'shr:dmodel_readLBUB'
  if (present(istr)) then
    lstr = trim(istr)//':'
  endif

  call t_startf(trim(lstr)//'setup')
  call MPI_COMM_RANK(mpicom,my_task,ierr)
  master_task = 0
  spd = shr_const_cday
  ...
  call t_stopf(trim(lstr)//'setup')

  if (rDateM < rDateLB .or. rDateM > rDateUB) then
     call t_startf(trim(lstr)//'fbound')
     if (my_task == master_task) then
!       call shr:stream_findBounds(stream,mDate,mSec,                 &
...
!                                  mDateUB,dDateUB,mSecUB,n_ub,fn_ub  )
     ...
     call t_stopf(trim(lstr)//'fbound')
     call t_startf(trim(lstr)//'bcast')
...
!     call shr_mpi_bcast(mSecUB,mpicom)
  ...
  call t_startf(trim(lstr)//'filemgt')
  ...
  call t_stopf(trim(lstr)//'filemgt')
...
!-------------------------------------------------------------------------------
  ...
  lstr = 'shr:dmodel_readstrm'
  if (present(istr)) then
    lstr = trim(istr)//':'
  endif

  call t_barrierf(trim(lstr)//'BARRIER',mpicom)
  call t_startf(trim(lstr)//'setup')
  call MPI_COMM_RANK(mpicom,my_task,ierr)
  master_task = 0
  ...
  if (pio_iotype == iotype_std_netcdf) then
     ...
     call t_startf(trim(lstr)//'readcdf')
     if (my_task == master_task) then
     ...
     call t_stopf(trim(lstr)//'readcdf')
     call t_barrierf(trim(lstr)//'scatter'//'_BARRIER',mpicom)
     call t_startf(trim(lstr)//'scatter')
     ...
     call t_stopf(trim(lstr)//'scatter')

  else

     call t_startf(trim(lstr)//'readpio')
     ...
     call t_stopf(trim(lstr)//'readpio')
