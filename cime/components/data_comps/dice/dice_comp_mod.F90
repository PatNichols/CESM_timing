...
!-------------------------------------------------------------------------------
   ...
   &                          scmlat=scmlat, scmlon=scmLon)
    ...
    call t_stopf('dice:readnml')

    !----------------------------------------------------------------------------
    ! Initialize datasets
    !----------------------------------------------------------------------------

    call t_startf('dice:strdata_init')
    ...
    call t_stopf('dice:strdata_init')
    ...
    call t_stopf('dice:initgsmaps')

    !----------------------------------------------------------------------------
    ! Initialize MCT domain
    !----------------------------------------------------------------------------

    call t_startf('dice:initmctdom')
    ...
    call t_stopf('dice:initmctdom')

    !----------------------------------------------------------------------------
    ! Initialize MCT attribute vectors
    !----------------------------------------------------------------------------

    call t_startf('dice:initmctavs')
...
!-------------------------------------------------------------------------------
   ...
   call t_startf('dice:RUN')

   call t_startf('dice:run1')
  ...
  !----------------------------------------------------------------------------
   ...
   call t_stopf('dice:run1')

   !--------------------
   ! UNPACK
   !--------------------

   call t_startf('dice:unpack')

!  lsize = mct_avect_lsize(x2i)

   call t_stopf('dice:unpack')
...
!         iFrac0(n) = i2x%rAttr(kiFrac,n)
   ...
   call t_stopf('dice:mode')

   if (write_restart) then
      call t_startf('dice:restart')
      ...
      call shr_sys_flush(logunit)
      call t_stopf('dice:restart')
   endif

   call t_stopf('dice:dice')
   ...
   call shr_file_setLogLevel(shrloglev)
   call shr_sys_flush(logunit)
   call t_stopf('dice:run2')

   call t_stopf('dice:RUN')
