...
!-------------------------------------------------------------------------------
 ...
 if (phase == 1) then
   ...
   &                          scmlat=scmlat, scmlon=scmLon)
    ...
    call t_stopf('datm:readnml')

    !----------------------------------------------------------------------------
    ! Initialize datasets
    !----------------------------------------------------------------------------

    call t_startf('datm:strdata_init')
    ...
    call t_stopf('datm:strdata_init')
    ...
    call t_stopf('datm:initgsmaps')

    !----------------------------------------------------------------------------
    ! Initialize MCT domain
    !----------------------------------------------------------------------------

    call t_startf('datm:initmctdom')
    ...
    call t_stopf('datm:initmctdom')

    !----------------------------------------------------------------------------
    ! Initialize MCT attribute vectors
    !----------------------------------------------------------------------------

    call t_startf('datm:initmctavs')
...
!-------------------------------------------------------------------------------
   ...
   call t_startf('datm:RUN')

   call t_startf('datm:run1')
  ...
  !----------------------------------------------------------------------------
   ...
   call t_stopf('datm:run1')

   !--------------------
   ! ADVANCE ATM
   !--------------------

   call t_barrierf('datm:BARRIER',mpicom)
   call t_startf('datm')
   ...
   if (trim(atm_mode) /= 'NULL') then
      call t_startf('datm:strdata_advance')
      call shr_strdata_advance(SDATM,currentYMD,currentTOD,mpicom,'datm')
      call t_stopf('datm:strdata_advance')
      call t_barrierf('datm:scatter_BARRIER',mpicom)
      call t_startf('datm:scatter')
      ...
      call t_stopf('datm:scatter')
   else
      call mct_aVect_zero(a2x)
   endif

   call t_startf('datm:mode')
   ...
   call shr_file_setLogLevel(shrloglev)
   call shr_sys_flush(logunit)
   call t_stopf('datm:run2')

   call t_stopf("datm:RUN')
