...
!-------------------------------------------------------------------------------
   ...
   &                          scmlat=scmlat, scmlon=scmLon)
    ...
    call t_stopf('docn:readnml')

    !----------------------------------------------------------------------------
    ! Initialize datasets
    !----------------------------------------------------------------------------

    call t_startf('docn:strdata_init')
    ...
    call t_stopf('docn:strdata_init')
    ...
    call t_stopf('docn:initgsmaps')

    !----------------------------------------------------------------------------
    ! Initialize MCT domain
    !----------------------------------------------------------------------------

    call t_startf('docn:initmctdom')
    ...
    call t_stopf('docn:initmctdom')

    !----------------------------------------------------------------------------
    ! Initialize MCT attribute vectors
    !----------------------------------------------------------------------------

    call t_startf('docn:initmctavs')
...
!-------------------------------------------------------------------------------
   ...
   call t_startf('docn:RUN')

   call t_startf('docn:run1')
  ...
  !----------------------------------------------------------------------------
   ...
   call t_stopf('docn:run1')

   !--------------------
   ! UNPACK
   !--------------------

   call t_startf('docn:unpack')
...
!   enddo
   ...
   call t_stopf('docn:unpack')

   !--------------------
   ! ADVANCE OCN
   !--------------------

   call t_barrierf('docn:BARRIER',mpicom)
   call t_startf('docn:docn')
   ...
   if (trim(ocn_mode) /= 'NULL') then
      ...
      !--- copy streams to o2x ---
      call t_startf('docn:strdata_advance')
      call shr_strdata_advance(SDOCN,currentYMD,currentTOD,mpicom,'docn')
      call t_stopf('docn:strdata_advance')
      call t_barrierf('docn:scatter_BARRIER',mpicom)
      call t_startf('docn:scatter')
      do n = 1,SDOCN%nstreams
         call shr_dmodel_translateAV(SDOCN%avs(n),o2x,avifld,avofld,rearr)
      enddo
      call t_stopf('docn:scatter')
   else
      call mct_aVect_zero(o2x)
   endif

   call t_startf('docn:mode')
   ...
   call t_stopf('docn:mode')

   if (write_restart) then
      call t_startf('docn:restart')
      ...
      call shr_sys_flush(logunit)
      call t_stopf('docn:restart')
   endif

   call t_stopf('docn:docn')
   ...
   call shr_file_setLogLevel(shrloglev)
   call shr_sys_flush(logunit)
   call t_stopf('docn:run2')

   call t_stopf('docn:RUN')
