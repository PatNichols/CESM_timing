...
!    endif
    ...
    call t_startf('mosart:i_dec_basins')
    idxocn  = 0
    nupstrm = 0
    do nr=1,rtmlon*rtmlat
       n = nr
    ...
    enddo
    call t_stopf('mosart:i_dec_basins')
...
!      !if (masterproc) write(iulog,*) 'nupstrm check ',nr,gmask(nr),nupstrm(nr),idxocn(nr)
    ...
    call t_startf('mosart:i_dec_distr')
    ...
    deallocate(pocn)
    call t_stopf('mosart:i_dec_distr')
    ...
    call t_stopf('mosart:i_print')

    !-------------------------------------------------------
    ! Allocate local flux variables
    !-------------------------------------------------------

    call t_startf('mosart:i_vars')
...
!    if (masterproc) call shr_sys_flush(iulog)
    ...
    call t_stopf('mosart:i_vars')

    !-------------------------------------------------------
    ! Initialize mosart
    !-------------------------------------------------------

    call t_startf('mosart:i_mosart_init')
...
!    if (masterproc) call shr_sys_flush(iulog)
    ...
    call t_stopf('mosart:i_mosart_init')

    !-------------------------------------------------------
    ! Read restart/initial info
    !-------------------------------------------------------

    call t_startf('mosart:i_restart')
...
!       enddo
    ...
    call t_stopf('mosart:i_restart')

    !-------------------------------------------------------
    ! Initialize mosart history handler and fields
    !-------------------------------------------------------

    call t_startf('mosart:i_histinit')
...
!    if (masterproc) call shr_sys_flush(iulog)
    ...
    call t_stopf('mosart:i_histinit')
...
!    endif
    ...
    if (barrier_timers) then
       call t_startf('mosart:r_SMdirect_barrier')
       call mpi_barrier(mpicom_rof,ier)
       call t_stopf ('mosart:r_SMdirect_barrier')
    endif

    call t_startf('mosart:r_SMdirect')
    ...
    do nr = rtmCTL%begr,rtmCTL%endr
       ...
       enddo
    enddo
    call t_stopf('mosart:r_SMdirect')

    !-----------------------------------
    ! MOSART Subcycling
    !-----------------------------------

    call t_startf('mosart:r_subcycling')
...
!    endif
    ...
    do ns = 1,nsub
       ...
       call t_startf('mosart:r_euler')
       call Euler()
       call t_stopf('mosart:r_euler')
...
!
!       if (barrier_timers) then
!          call t_startf('mosart:r_SMdnstrm_barrier')
!          call mpi_barrier(mpicom_rof,ier)
!          call t_stopf ('mosart:r_SMdnstrm_barrier')
!       endif
!
!       call t_startf('mosart:r_SMdnstrm')
!
...
!          enddo
!       enddo
!       call t_stopf('mosart:r_SMdnstrm')
    ...
    call t_stopf('mosart:r_subcycling')
...
!    if (budget_check) then
    ...
    call t_startf('mosart:r_hbuf')
    call RtmHistFldsSet()
    call RtmHistUpdateHbuf()
    call t_stopf('mosart:r_hbuf')

    call t_startf('mosart:r_htapes')
    call RtmHistHtapesWrapup( rstwr, nlend )
    call t_stopf('mosart:r_htapes')
