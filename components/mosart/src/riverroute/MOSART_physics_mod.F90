...
!-----------------------------------------------------------------------
  ...
  ! !DESCRIPTION: solve the ODEs with Euler algorithm
    ...
    call t_startf('mosart:r_hillslope')
    do nt=1,nt_rtm
    ...
    call t_stopf('mosart:r_hillslope')
    ...
    do m=1,Tctl%DLevelH2R
       ...
       call t_startf('mosart:r_subnetwork')    
       ...
       call t_stopf('mosart:r_subnetwork')    
       ...
       if (barrier_timers) then
          call t_startf('mosart:r_SMeroutUp_barrier')    
          call mpi_barrier(mpicom_rof,ier)
          call t_stopf('mosart:r_SMeroutUp_barrier')    
       endif

       call t_startf('mosart:r_SMeroutUp')    
       ...
       enddo
#endif
       call t_stopf('mosart:r_SMeroutUp')    
       ...
       call t_startf('mosart:r_chanroute')    
       do nt=1,nt_rtm
       if (TUnit%euler_calc(nt)) then
       do iunit=rtmCTL%begr,rtmCTL%endr
          if(TUnit%mask(iunit) > 0) then
...
!                end if
       ...
       call t_stopf('mosart:r_chanroute')    
    end do
