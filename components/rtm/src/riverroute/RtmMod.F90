...
!-----------------------------------------------------------------------
    ...
    call t_startf('rtm:i_dec_basins')
    iocn = 0
    nocn = 0
    ...
    enddo
    call t_stopf('rtm:i_dec_basins')

    !-------------------------------------------------------
    !--- Now allocate those basins to pes
    !-------------------------------------------------------

    call t_startf('rtm:i_dec_distr')
    ...
    call t_stopf('rtm:i_print')

    !-------------------------------------------------------
    ! Allocate local flux variables
    !-------------------------------------------------------

    call t_startf('rtm:i_vars')
...
!-----------------------------------------------------------------------
    ...
    call t_startf('RTM:hbuf')
    call RtmHistFldsSet()
    call RtmHistUpdateHbuf()
    call t_stopf('RTM:hbuf')

    call t_startf('RTM:htapes')
    call RtmHistHtapesWrapup( rstwr, nlend )
    call t_stopf('RTM:htapes')

    if (rstwr) then
       call t_startf('RTM:rest')
       filer = RtmRestFileName(rdate=rdate)
       call RtmRestFileWrite( filer, rdate=rdate )
       call t_stopf('RTM:rest')
    end if
