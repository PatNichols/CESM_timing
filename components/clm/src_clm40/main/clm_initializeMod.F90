...
!----------------------------------------------------------------------
    ...
    call t_stopf('clm:init_ecosys')

    ! Initialize dust emissions model 

    call t_startf('clm:init_dust')
    call Dustini()
    call t_stopf('clm:init_dust')
    ...
    ! ------------------------------------------------------------------------
    call t_startf('clm:hist_initFlds')
    ...
    call t_stopf('clm:hist_initFlds')
    ! ------------------------------------------------------------------------
    ...
    call t_startf('clm:init_accflds')
    call initAccFlds()
    call t_stopf('clm:init_accflds')
    ...
    if (use_cn) then
       call t_startf('clm:init_cninitim')
       if (nsrest == nsrStartup) then
          call CNiniTimeVar()
       end if
       call t_stopf('clm:init_cninitim')
    end if
    ...
    if (use_cndv) then
       ...
       if (flanduse_timeseries /= ' ') then
          call t_startf('clm:init_pftdyn')
          call pftdyn_init()
          call pftdyn_interp( )
          call t_stopf('clm:init_pftdyn')
       end if
    end if
    ...
    if (use_cn) then
       call t_startf('clm:init_ndep')
       call ndep_init()
       call ndep_interp()
       call t_stopf('clm:init_ndep')
    end if
    ...
    call t_startf('clm:init_filters')
...
!$OMP END PARALLEL DO
    ...
    if (get_nstep() == 0 .or. nsrest == nsrStartup) then
       ...
       if (finidat == ' ' .or. do_initsurfalb) then
          call t_startf('clm:init_orb')
          calday = get_curr_calday()
          call t_startf('clm:init_orbd1')
          call shr_orb_decl( calday, eccen, mvelpp, lambm0, obliqr, declin, eccf )
          call t_stopf('clm:init_orbd1')
          
          dtime = get_step_size()
          caldaym1 = get_curr_calday(offset=-int(dtime))
          call t_startf('clm:init_orbd2')
          call shr_orb_decl( caldaym1, eccen, mvelpp, lambm0, obliqr, declinm1, eccf )
          call t_stopf('clm:init_orbd2')
          
          call t_startf('clm:init_orbSA')
          call initSurfAlb( calday, declin, declinm1 )
          call t_stopf('clm:init_orbSA')
          call t_stopf('clm:init_orb')
       ...
       call t_startf('clm:init_map2gc')
       call clm_map2gcell_minimal()
       call t_stopf('clm:init_map2gc')

    end if

    ! Initialize sno export state
    if (create_glacier_mec_landunit) then
       call t_startf('clm:init_create_s2x')
       call update_clm_s2x(init=.true.)
       call t_stopf('clm:init_create_s2x')
    end if

    call t_stopf('clm:clm_init2')
