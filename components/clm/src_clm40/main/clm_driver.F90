...
!-----------------------------------------------------------------------
  ...
  call t_startf('clm:clm_map2gcell')
  call clm_map2gcell( )
  call t_stopf('clm:clm_map2gcell')
  ...
  if (create_glacier_mec_landunit) then
     call t_startf('clm:create_s2x')
     call update_clm_s2x(init=.false.)
     call t_stopf('clm:create_s2x')
  end if
  ...
  if (use_cndv) then
     if (monp1==1 .and. dayp1==1 .and. secp1==dtime .and. nstep>0)  then
        call t_startf('clm:clm_drv_io_hdgvm')
        call histCNDV()
        if (masterproc) write(iulog,*) 'Annual CNDV calculations are complete'
        call t_stopf('clm:clm_drv_io_hdgvm')
     end if
  end if
