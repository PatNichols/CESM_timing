...
contains
  ...
  subroutine clm_drv(doalb, nextsw_cday, declinp1, declin, rstwr, nlend, rdate)
    ...
    if (use_cn) then 
       ...
       if ( n_drydep > 0 .and. drydep_method == DD_XLND ) then
          call t_startf('clm:interpMonthlyVeg')
          call interpMonthlyVeg(bounds_proc, canopystate_inst)
          call t_stopf('clm:interpMonthlyVeg')
       endif
    ...
    do nc = 1,nclumps
       ...
       call CanopyHydrology(bounds_clump, &
            ...
            irrigation_inst)
       call t_stopf('clm:canhydro')

       ! ============================================================================
       ! Surface Radiation
       ! ============================================================================

       call t_startf('clm:surfrad')
       ...
       call t_stopf('clm:bgp1')

       ! ============================================================================
       ! Determine fluxes
       ! ============================================================================

       call t_startf('clm:bgflux')
       ...
       call t_stopf('clm:canflux')

       ! Fluxes for all urban landunits

       call t_startf('clm:uflux')
       call UrbanFluxes(bounds_clump,                                         &
            ...
            humanindex_inst)
       call t_stopf('clm:uflux')

       ! Fluxes for all lake landunits

       call t_startf('clm:bgplake')
       ...
       call t_stopf('clm:bgplake')

       ! Set soil/snow temperatures including ground temperature

       call t_startf('clm:soiltemperature')
       ...
       call t_stopf('clm:soiltemperature')

       ! ============================================================================
       ! update surface fluxes for new ground temperature.
       ! ============================================================================

       call t_startf('clm:bgp2')
       ...
       call t_startf('clm:hydro without drainage')
       ...
       call t_stopf('clm:hydro without drainage')
       ...
       call t_startf('clm:snow_init')
       call SnowAge_grain(bounds_clump,                 &
            filter(nc)%num_snowc, filter(nc)%snowc,     &
            filter(nc)%num_nosnowc, filter(nc)%nosnowc, &
            waterflux_inst, waterstate_inst, temperature_inst)
       call t_stopf('clm:snow_init')
       ...
       call t_stopf('clm:depvel')

       ! Calculation of methane fluxes

       if (use_lch4) then
          call t_startf('clm:ch4')
       ...
       call t_startf('clm:hydro2 drainage')
       ...
       call t_stopf('clm:hydro2 drainage')     
    ...
    call t_startf('clm:hbuf')
    call hist_update_hbuf(bounds_proc)
    call t_stopf('clm:hbuf')
    ...
    if (.not. use_noio) then
       ...
       call t_startf('clm:clm_drv_io')

       ! Create history and write history tapes if appropriate
       call t_startf('clm:clm_drv_io_htapes')
       ...
       call t_stopf('clm:clm_drv_io_htapes')
       ...
       if (rstwr) then
          call t_startf('clm:clm_drv_io_wrest')
          filer = restFile_filename(rdate=rdate)

          call restFile_write( bounds_proc, filer, rdate=rdate )

          call t_stopf('clm:clm_drv_io_wrest')
       end if
       call t_stopf('clm:clm_drv_io')
  ...
  subroutine write_diagnostic (bounds, wrtdia, nstep, lnd2atm_inst)
    ...
    if (wrtdia) then
       ...
       call t_barrierf('clm:sync_write_diag', mpicom)
