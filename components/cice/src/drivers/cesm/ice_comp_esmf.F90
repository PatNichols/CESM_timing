...
!-----------------------------------------------------------------------
    ...
    call t_barrierf('cice:init_total_BARRIER',MPI_COMM_ICE)
    call t_startf ('cice:init_total')
    ...
    !=============================================================
    ! Initialize cice_ because grid information is needed for
    ! creation of gsmap_i.  cice__init also sets time manager info
    !=============================================================
    ...
    call t_startf ('cice:init')
    call cice__init( mpicom_loc )
    call t_stopf ('cice:init')
    ...
    if (runtype == 'initial') then
       ...
       if (my_task == master_task) then
          ...
          write(nu_diag,*) trim(subname),' cice_ year_init = ',year_init
          write(nu_diag,*) trim(subname),' cice_ start date = ',idate
          write(nu_diag,*) trim(subname),' cice_ start ymds = ',iyear,month,mday,start_tod
       endif
...
! not explicitly setting it here makes cice_5 work in cesm1.1
...
!---------------------------------------------------------------------------
    ...
    call t_barrierf('cice:run_total_BARRIER',MPI_COMM_ICE)
    call t_startf ('cice:run_total')
    ...
    call t_barrierf('cice:run_import_BARRIER',MPI_COMM_ICE)
    call t_startf ('cice:run_import')
    ...
    call t_barrierf('cice:run_export_BARRIER',MPI_COMM_ICE)
    call t_startf ('cice:run_export')
    ...
    if (.not. seq_timemgr_EClockDateInSync( EClock, ymd, tod )) then
       call seq_timemgr_EClockGetData( EClock, curr_ymd=ymd_sync, &
          curr_tod=tod_sync )
       write(nu_diag,*)' cice_ ymd=',ymd     ,'  cice_ tod= ',tod
