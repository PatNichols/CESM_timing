...
contains
  ...
  subroutine ice_import( x2i )
    ...
    if (.not.prescribed_ice) then
       call t_startf ('cice:imp_halo')
       call ice_HaloUpdate(aflds, halo_info, field_loc_center, &
            field_type_scalar)
       call t_stopf ('cice:imp_halo')
    endif
