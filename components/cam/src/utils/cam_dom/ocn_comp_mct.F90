...
!==========================================================================
  ...
  subroutine ocn_run_mct ( EClock, cdata_o, x2o_o, o2x_o)
    ...
    if (rstwr) then
       call t_startf('atm:dom:ocn_write_restart')
       ...
       call t_stopf('atm:dom:ocn_write_restart')
    end if

    ! Run ocean model

    call t_startf('atm:dom:ocn_run')
    call ocn_run( ocn_out )
    call t_stopf('atm:dom:ocn_run')

    ! Extract export state

    call t_startf('atm:dom:ocn_export')
    call ocn_export_mct (ocn_out, o2x_o )
    call t_stopf('atm:dom:ocn_export')
