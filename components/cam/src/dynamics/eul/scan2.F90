...
!-----------------------------------------------------------------------
   if (.not. single_column) then
      ...
      call t_startf('atm:dyn:eu:grcalc')
...
!$OMP PARALLEL DO PRIVATE (LAT, J)
   ...
   call t_stopf('atm:dyn:eu:grcalc')

   call t_startf('atm:dyn:eu:spegrd_alloc')
#if ( defined SPMD )
   nlon_fft_in = 2*maxm
   ...
   call t_stopf('atm:dyn:eu:spegrd_alloc')
!
   call t_startf('atm:dyn:eu:spegrd_bft')
!$OMP PARALLEL DO PRIVATE (LAT, IROW)
   ...
   call t_stopf('atm:dyn:eu:spegrd_bft')

   call t_startf('atm:dyn:eu:spegrd_ift')
   call spegrd_ift ( nlon_fft_in, nlon_fft_out, fftbuf_in, fftbuf_out )
   call t_stopf('atm:dyn:eu:spegrd_ift')
                   
   call t_startf('atm:dyn:eu:spegrd_aft')
#ifdef OUTER_OMP
...
!
   call t_startf('atm:dyn:eu:scan2_single')
   tmassf = 0._r8
   do lat=1,plat
...
!
   ...
   call t_stopf('atm:dyn:eu:scan2_single')

   call t_startf('atm:dyn:eu:tfilt_massfix')
...
!$OMP PARALLEL DO PRIVATE (LAT)
   ...
   call t_stopf('atm:dyn:eu:tfilt_massfix')
!
! Shift time pointers
!
