...
if ( single_column ) then
   ...
   call t_startf('atm:dyn:eu:scandyn')
   ...
   call t_stopf('atm:dyn:eu:scandyn')
endif
!
!----------------------------------------------------------
...
!----------------------------------------------------------
!
   call t_startf('atm:dyn:eu:sltrun')
   call scanslt_run(adv_state, ztodt   ,etadot  , detam, etamid, cwava  )
   call t_stopf('atm:dyn:eu:sltrun')
...
!----------------------------------------------------------
!
   call t_startf('atm:dyn:eu:dynpkg_alloc')
   allocate( vz  (2*lpspt,plev) )
   allocate( d   (2*lpspt,plev) )
   allocate( t   (2*lpspt,plev) )
   allocate( alps(2*lpspt) )
   call t_stopf('atm:dyn:eu:dynpkg_alloc')

   call t_startf('atm:dyn:eu:dyndrv')
   call dyndrv(grlps1  ,grt1    ,grz1    ,grd1    ,grfu1   ,  &
               ...
               vcour, ztodt   )
   call t_stopf('atm:dyn:eu:dyndrv')
!
!----------------------------------------------------------
! Second gaussian scan (spectral -> grid)
!----------------------------------------------------------
!
   call t_startf('atm:dyn:eu:scan2')
   call scan2run (ztodt,   cwava,   etamid)
   call t_stopf('atm:dyn:eu:scan2')

   call t_startf('atm:dyn:eu:dynpkg_dealloc')
   deallocate( vz )
   deallocate( d )
   deallocate( t )
   deallocate( alps )
   call t_stopf('atm:dyn:eu:dynpkg_dealloc')
endif
