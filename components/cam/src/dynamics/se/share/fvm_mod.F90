...
!    real (kind=real_kind), dimension(1:nc,1:nc)                        :: tracer1, tracer_air1 !dbg
    ...
    call t_startf('atm:dyn:se:Fluxform-CSLAM scheme') 
...
!             recons=0.0
    ...
    call t_stopf('atm:dyn:se:Fluxform-CSLAM scheme')
    call t_startf('atm:dyn:se:FVM Communication')
    call ghost_exchangeV(hybrid,cellghostbuf,nhc,nc,ntrac+1)
    call t_stopf('atm:dyn:se:FVM Communication')
    !-----------------------------------------------------------------------------------!
    call t_startf('atm:dyn:se:FVM Unpack')
    do ie=nets,nete
    ...
    enddo
    call t_stopf('atm:dyn:se:FVM Unpack')
  ...
  subroutine cslam_runairdensity(elem,fvm,hybrid,deriv,tstep,tl,nets,nete)
    ...
    call initghostbufferTR(buflatlon,nlev,2,2,nc+1)    ! use the tracer entry 2 for lat lon
    call t_startf('atm:dyn:se:CSLAM scheme') 
    ...
    call t_stopf('atm:dyn:se:CSLAM scheme')
    call t_startf('atm:dyn:se:FVM Communication')
    call ghost_exchangeV(hybrid,cellghostbuf,nhc,nc,ntrac+1)
    call t_stopf('atm:dyn:se:FVM Communication')
    !-----------------------------------------------------------------------------------!                         
