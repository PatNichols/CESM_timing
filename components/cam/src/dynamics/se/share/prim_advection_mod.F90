...
subroutine remap1_nofilter(Qdp,nx,qsize,dp1,dp2)
  ...
  integer(kind=int_kind) :: zkr(nlev+1),filter_code(nlev),peaks,im1,im2,im3,ip1,ip2, &
                            lt1,lt2,lt3,t0,t1,t2,t3,t4,tm,tp,ie,i,ilev,j,jk,k,q
  logical :: abort=.false.
!   call t_startf('atm:dyn:se:remap1_nofilter')
...
!   call t_stopf('atm:dyn:se:remap1_nofilter')
...
subroutine remap_Q_ppm(Qdp,nx,qsize,dp1,dp2)
  ...
  call t_startf('atm:dyn:se:remap_Q_ppm')
  do j = 1 , nx
    do i = 1 , nx
    ...
    enddo
  enddo
  call t_stopf('atm:dyn:se:remap_Q_ppm')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine Prim_Advec_Tracers_spelt(elem, spelt, deriv,hvcoord,hybrid,&
    ...
    call t_startf('atm:dyn:se:prim_advec_tracers_spelt')
    np1 = tl%np1
...
!       call spelt_runlimit(elem,spelt,hybrid,deriv,dt,tl,nets,nete)
    call t_stopf('atm:dyn:se:prim_advec_tracers_spelt')
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine Prim_Advec_Tracers_fvm(elem, fvm, deriv,hvcoord,hybrid,&
    ...
    call t_startf('atm:dyn:se:prim_advec_tracers_fvm')
    np1 = tl%np1
...
!------------------------------------------------------------------------------------
    ...
    call t_stopf('atm:dyn:se:prim_advec_tracers_fvm')
...
!-----------------------------------------------------------------------------
  subroutine Prim_Advec_Tracers_remap_rk2( elem , deriv , hvcoord , flt , hybrid , dt , tl , nets , nete )
    ...
    call t_startf('atm:dyn:se:prim_advec_tracers_remap_rk2')
...
!$omp parallel do private(k, gradQ)
    ...
    call t_stopf('atm:dyn:se:prim_advec_tracers_remap_rk2')
...
!   call t_startf('atm:dyn:se:euler_step')
...
#endif
#endif
!   call t_stopf('atm:dyn:se:euler_step')
...
! nelemd
  ...
  call t_startf('atm:dyn:se:euler_step_dg')
...
#endif
#endif
  call t_stopf('atm:dyn:se:euler_step_dg')
...
!   call t_barrierf('sync_advance_hypervis_scalar', hybrid%par%comm)
  call t_startf('atm:dyn:se:advance_hypervis_scalar')
...
!$OMP BARRIER
...
#endif
  enddo
  call t_stopf('atm:dyn:se:advance_hypervis_scalar')
  ...
  call t_startf('atm:dyn:se:vertical_remap')
...
!         call remap_velocityC(np1,dt,elem,fvm,hvcoord,ie)
  ...
  enddo
  call t_stopf('atm:dyn:se:vertical_remap')
