...
contains
  ...
  function pcg_solver_nonstag(elem,       &
    ...
    call t_startf('atm:dyn:se:pcg_solver_nonstag')
...
!$omp parallel do private(k,i,j,iptr)
    ...
    do while (congrad(cg,red,maxits,tol))
       call t_startf('atm:dyn:se:timer_helm')
...
#endif
#endif
       call t_stopf('atm:dyn:se:timer_helm')
...
!$omp parallel do private(k,i,j,iptr)
    ...
    call t_stopf('atm:dyn:se:pcg_solver_nonstag')
#endif
...
!   
!  call build_rhs(nets,nete,np,nlev,rhsVector,c_ptr_helmholtzdata) 
!       call t_startf('atm:dyn:se:timer_trilinosmlhelm')
!  call helmholtz_solve(rhsVectorSize, solVector) 
!       call t_stopf('atm:dyn:se:timer_trilinosmlhelm')
