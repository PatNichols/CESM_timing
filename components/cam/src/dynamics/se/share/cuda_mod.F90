...
!$OMP BARRIER
  ...
  subroutine copy_qdp_h2d( elem , nt )
    ...
    !$OMP MASTER
    ierr = cudaThreadSynchronize()
    call t_startf('atm:dyn:se:cuda:CUDA QDP H2D')
    do ie = 1,nelemd
    ...
    enddo
    ierr = cudaThreadSynchronize()
    call t_stopf('atm:dyn:se:cuda:CUDA QDP H2D')
    !$OMP END MASTER
    !$OMP BARRIER
  ...
  subroutine copy_qdp_d2h( elem , nt )
    ...
    !$OMP MASTER
    ierr = cudaThreadSynchronize()
    call t_startf('atm:dyn:se:cuda:CUDA QDP D2H')
    do ie = 1,nelemd
    ...
    enddo
    ierr = cudaThreadSynchronize()
    call t_stopf('atm:dyn:se:cuda:CUDA QDP D2H')
    !$OMP END MASTER
    !$OMP BARRIER
  ...
  call t_startf('atm:dyn:se:cuda:euler_step')
...
!$OMP BARRIER
  ...
  call t_stopf('atm:dyn:se:cuda:euler_step')
...
subroutine advance_hypervis_scalar_cuda( edgeAdv , elem , hvcoord , hybrid , deriv , nt , nt_qdp , nets , nete , dt2 )
  ...
  call t_startf('atm:dyn:se:cuda:advance_hypervis_scalar_cuda')
...
!$OMP BARRIER
  ...
  call t_stopf('atm:dyn:se:cuda:advance_hypervis_scalar_cuda')
...
subroutine pack_exchange_unpack_stage(np1,hybrid,array_in,tl_in)
  ...
  ierr = cudaThreadSynchronize()
  call t_startf('atm:dyn:se:cuda:CUDA PEU NEWER')
  do icycle=1,nRecvCycles
  ...
  ierr = cudaThreadSynchronize()
  call t_stopf('atm:dyn:se:cuda:CUDA PEU NEWER')
...
subroutine vertical_remap_cuda(elem,fvm,hvcoord,dt,np1,np1_qdp,nets,nete)
  ...
  call t_startf('atm:dyn:se:cuda:vertical_remap_cuda')
  do ie=nets,nete
...
!$OMP END MASTER
!$OMP BARRIER
  call t_stopf('atm:dyn:se:cuda:vertical_remap_cuda')  
...
subroutine remap_Q_ppm_cuda(elem,nx,qsize,dp1,dp2,np1_qdp,ie)
  ...
  call t_startf('atm:dyn:se:cuda:remap_Q_ppm_cuda')
  ...
  call t_stopf('atm:dyn:se:cuda:remap_Q_ppm_cuda')
...
subroutine remap_Q_ppm(Qdp,nx,qsize,dp1,dp2)
  ...
  call t_startf('atm:dyn:se:cuda:remap_Q_ppm')
  do j = 1 , nx
    do i = 1 , nx
    ...
    enddo
  enddo
  call t_stopf('atm:dyn:se:cuda:remap_Q_ppm')
