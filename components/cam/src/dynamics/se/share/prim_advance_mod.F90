...
!JMD    call t_barrierf('sync_prim_advance_exp', hybrid%par%comm)
    ...
    call t_startf('atm:dyn:se:prim_advance_exp')
    nm1   = tl%nm1
    n0    = tl%n0
    np1   = tl%np1
    nstep = tl%nstep
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
    if (1==prescribed_wind) then
	  ...
	  !get eta_dot_dpdn at n0, ps_v is not used or calculated in this routine
       ...
       call t_stopf('atm:dyn:se:prim_advance_exp')
       call t_adj_detailf(-1)
       return
    endif
...
!JMD       call t_barrierf('sync_prim_advance_si', hybrid%par%comm)
       call t_adj_detailf(+1)
       call t_startf('atm:dyn:se:prim_advance_si')
...
!$OMP BARRIER
...
#endif
       call t_stopf('atm:dyn:se:prim_advance_si')
       call t_adj_detailf(-1)
#endif
  ...
  call t_adj_detailf(+1)
  call t_startf('atm:dyn:se:preq_robert')
  do ie=nets,nete
     if (filter_ps) then
  ...
  call t_stopf('atm:dyn:se:preq_robert')
  call t_adj_detailf(-1)
...
!JMD  call t_barrierf('sync_advance_hypervis', hybrid%par%comm)
  call t_adj_detailf(+1)
  call t_startf('atm:dyn:se:advance_hypervis')
...
!$OMP BARRIER
  ...
  call t_stopf('atm:dyn:se:advance_hypervis')
  call t_adj_detailf(-1)
...
!JMD  call t_barrierf('sync_advance_hypervis', hybrid%par%comm)
  call t_adj_detailf(+1) 
  call t_startf('atm:dyn:se:advance_hypervis_dp')
...
!$OMP BARRIER
  ...
  call t_stopf('atm:dyn:se:advance_hypervis_dp')
  call t_adj_detailf(-1)
...
!JMD  call t_barrierf('sync_advance_hypervis_lf', hybrid%par%comm)
  call t_adj_detailf(+1)
  call t_startf('atm:dyn:se:advance_hypervis_lf')
...
!$OMP BARRIER
  ...
  call t_stopf('atm:dyn:se:advance_hypervis_lf')
  call t_adj_detailf(-1)
...
!JMD  call t_barrierf('sync_compute_and_apply_rhs', hybrid%par%comm)
  call t_adj_detailf(+1)
  call t_startf('atm:dyn:se:compute_and_apply_rhs')
  do ie=nets,nete
...
#endif
#endif
  call t_stopf('atm:dyn:se:compute_and_apply_rhs')
  call t_adj_detailf(-1)
...
!JMD  call t_barrierf('sync_residual', hybrid%par%comm)
  call t_adj_detailf(+1)
  call t_startf('atm:dyn:se:residual')

     call t_startf('atm:dyn:se:residual_int')
     !fvtens = 0.0d0
     !fttens = 0.0d0
     !fpstens = 0.0d0
     !fdptens = 0.0d0
	   ...
	   call t_stopf('atm:dyn:se:residual_int')
       
	   call t_startf('atm:dyn:se:residual_cal')
...
!$omp parallel do private(k)
  ...
  call t_stopf('atm:dyn:se:residual_cal')
  call t_startf('atm:dyn:se:residual_fin')
	  lx = 1
	  do ie=nets,nete
		  do k=1,nlev
			  do j=1,np
      ...
      call t_stopf('atm:dyn:se:residual_fin')
...
#endif
#endif
  call t_stopf('atm:dyn:se:residual')
  call t_adj_detailf(-1)
...
!    call t_startf('atm:dyn:se:test id')
!call flush(6)
!write(6,*)'startf'
!call flush(6)
...
!    call t_stopf('atm:dyn:se:test id')
