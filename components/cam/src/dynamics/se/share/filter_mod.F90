...
contains
  ...
  function taylor_filter_create(Tp, mu, gquadp) result(flt)
    ...
    call t_startf('atm:dyn:se:taylor_filter_create')
    allocate(gammaP(np))
    ...
    deallocate(AP)
    deallocate(AinvP)
    call t_stopf('atm:dyn:se:taylor_filter_create')
  ...
  function fm_filter_create(Tp, mu, gquadP) result(flt)
    ...
    call t_startf('atm:dyn:se:fm_filter_create')
    ...
    deallocate(legP)
    deallocate(AP)
    call t_stopf('atm:dyn:se:fm_filter_create')
  ...
  function fm_transfer(kcut,wght,npts) result(T)
    ...
    call t_startf('atm:dyn:se:fm_transfer')
    kc  = npts-kcut
    cut = kcut
    ...
    call t_stopf('atm:dyn:se:fm_transfer')
  ...
  function bv_transfer(p,s,npts) result(T)
    ...
    call t_startf('atm:dyn:se:bv_transfer')
    do k=1,npts
    ...
    call t_stopf('atm:dyn:se:bv_transfer')
  ...
  function bvsigma(p,x) result(sigma)
    ...
    call t_startf('atm:dyn:se:bvsigma')
    ...
    call t_stopf('atm:dyn:se:bvsigma')
...
11     format(e22.15,3x,e22.15)
  ...
  subroutine preq_filter(elem, edge3p1, flt, hybrid, nfilt, nets, nete)
    ...
    call t_barrierf('atm:dyn:se:sync_preq_filter', hybrid%par%comm)
    call t_startf('atm:dyn:se:preq_filter')
    if (flt%type == "bv") then
       do ie=nets,nete          
...
!$OMP BARRIER
    ...
    call t_stopf('atm:dyn:se:preq_filter')
  ...
  subroutine prim_filter(elem, edge_3lp1,flt,nfilt,nets,nete,hybrid)
    ...
    call t_barrierf('atm:dyn:se:sync_prim_filter', hybrid%par%comm)
    call t_startf('atm:dyn:se:prim_filter')
...
!$omp parallel do private(k,i,j,v1,v2,u,v)
    ...
    call t_stopf('atm:dyn:se:prim_filter')
