...
!-----------------------------------------------------------------------
   ...
   call t_barrierf('atm:dyn:se:sync_d_p_coupling', mpicom)
   call t_startf('atm:dyn:se:d_p_coupling')
   call d_p_coupling (phys_state, phys_tend,  pbuf2d, dyn_out )
   call t_stopf('atm:dyn:se:d_p_coupling')
...
subroutine stepon_run2(phys_state, phys_tend, dyn_in, dyn_out )
   ...
   call t_barrierf('atm:dyn:se:sync_p_d_coupling', mpicom)
   call t_startf('atm:dyn:se:p_d_coupling')
   call p_d_coupling(phys_state, phys_tend,  dyn_in)
   call t_stopf('atm:dyn:se:p_d_coupling')

   if(iam >= par%nprocs) return

   call t_startf('atm:dyn:se:bndry_exchange')
...
!$omp parallel do private(k, j, i, dp_tmp)
   ...
   call t_stopf('atm:dyn:se:bndry_exchange')
...
subroutine stepon_run3(dtime, cam_out, phys_state, dyn_in, dyn_out)
   ...
   call t_barrierf('atm:dyn:se:sync_dyn_run', mpicom)
   call t_startf('atm:dyn:se:dyn_run')
   call dyn_run(dyn_out,rc)	
   call t_stopf('atm:dyn:se:dyn_run')
