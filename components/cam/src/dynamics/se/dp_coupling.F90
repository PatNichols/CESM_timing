...
! LOCAL VARIABLES
    ...
    if( iam < par%nprocs) then
       ...
       call t_startf('atm:dyn:se:UniquePoints')
       do ie=1,nelemd
       ...
       call t_stopf('atm:dyn:se:UniquePoints')
    ...
    call t_startf('atm:dyn:se:dpcopy')
...
!$omp parallel do private (ie, bpter, icol, ilyr, m)
       ...
       call t_barrierf ('atm:dyn:se:sync_blk_to_chk', mpicom)
       call t_startf('atm:dyn:se:block_to_chunk')
       call transpose_block_to_chunk(tsize, bbuffer, cbuffer)
       call t_stopf('atm:dyn:se:block_to_chunk')
...
!$omp parallel do private (lchnk, ncols, cpter, icol, ilyr, m, pbuf_chnk, pbuf_frontgf, pbuf_frontga)
    ...
    call t_stopf('atm:dyn:se:dpcopy')

    call t_startf('atm:dyn:se:derived_phys')
    call derived_phys(phys_state,phys_tend,pbuf2d)
    call t_stopf('atm:dyn:se:derived_phys')
...
!   real (kind=real_kind)    :: omega_tmp(npsq,pver,nelemd)   ! temporary array to hold omega
    ...
    call t_startf('atm:dyn:se:pd_copy')
    if(local_dp_map) then
...
!$omp parallel do private (lchnk, ncols, cpter, i, icol, ilyr, m)
       ...
       call t_barrierf('atm:dyn:se:sync_chk_to_blk', mpicom)
       call t_startf('atm:dyn:se:chunk_to_block')
       call transpose_chunk_to_block(tsize, cbuffer, bbuffer)
       call t_stopf('atm:dyn:se:chunk_to_block')
       if(iam < par%nprocs) then
!$omp parallel do private (ie, bpter, icol, ilyr, m)
          do ie=1,nelemd
    ...
    call t_stopf('atm:dyn:se:pd_copy')
    if(iam < par%nprocs) then
       call t_startf('atm:dyn:se:putUniquePoints')
       do ie=1,nelemd
          ncols = elem(ie)%idxP%NumUniquePts
       ...
       call t_stopf('atm:dyn:se:putUniquePoints')
    end if
