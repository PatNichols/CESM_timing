...
!BOC
   ...
   call t_barrierf('atm:dyn:fv:sync_diag_dynvar_ic', mpicom)
   call t_startf('atm:dyn:fv:diag_dynvar_ic')
   call diag_dynvar_ic (dyn_state%grid, dyn_out%phis, dyn_out%ps,             &
                        dyn_out%t3, dyn_out%u3s, dyn_out%v3s, dyn_out%tracer  )
   call t_stopf  ('atm:dyn:fv:diag_dynvar_ic')

   call t_startf('atm:dyn:fv:comp_adv_tends1')
   call compute_adv_tends_xyz(dyn_state%grid, dyn_in%tracer )
   call t_stopf  ('atm:dyn:fv:comp_adv_tends1')
   !
   !--------------------------------------------------------------------------
   ...
   !----------------------------------------------------------
   call t_barrierf('atm:dyn:fv:sync_dyn_run', mpicom)
   call t_startf('atm:dyn:fv:dyn_run')
   ...
   call t_stopf('atm:dyn:fv:dyn_run')

   call t_startf('atm:dyn:fv:comp_adv_tends2')
   call compute_adv_tends_xyz(dyn_state%grid, dyn_out%tracer )
   call t_stopf ('atm:dyn:fv:comp_adv_tends2')

   !----------------------------------------------------------
   ! Move data into phys_state structure.
   !----------------------------------------------------------
   call t_barrierf('atm:dyn:fv:sync_d_p_coupling', mpicom)
   call t_startf('atm:dyn:fv:d_p_coupling')
   call d_p_coupling(dyn_state%grid, phys_state, phys_tend,  pbuf2d, dyn_out)
   call t_stopf('atm:dyn:fv:d_p_coupling')
...
!-----------------------------------------------------------------------
   ...
   call t_barrierf('atm:dyn:fv:sync_p_d_coupling', mpicom)
   call t_startf('atm:dyn:fv:p_d_coupling')
   call p_d_coupling(grid, phys_state, phys_tend, &
                     dyn_in, dtime, zvir, cappa, ptop)
   call t_stopf('atm:dyn:fv:p_d_coupling')
...
!
   ...
   if ( fv_monitor .and. mod(ncsecp, freq_diag) == 0 ) then
      ...
      call t_barrierf('atm:dyn:fv:sync_fv_out', mpicom)
      call t_startf('atm:dyn:fv:fv_out')
      ...
      call t_stopf('atm:dyn:fv:fv_out')
   endif
