...
!$omp  parallel do private(i,k)
         ...
         if ( shr_reprosum_tolExceeded('d2a3dikj/South Pole', 2*km, write_warning, &
            ...
            if ( shr_reprosum_recompute ) then
               call t_startf("atm:dyn:fv:d2a3dikj_sumfix")
               allocate( uva_tmp(im) )
               do m = 1,2
                  do k = 1,km
               ...
               enddo
               deallocate( uva_tmp )
               call t_stopf("atm:dyn:fv:d2a3dikj_sumfix")
            endif
         endif
