...
!BOC
  ...
  if ( shr_reprosum_tolExceeded('gmeanxy', 1, write_warning, &
     ...
     if ( shr_reprosum_recompute ) then
        call t_startf("atm:dyn:fv:gmeanxy_sumfix")
        ...
        deallocate( q_global )
        call t_stopf("atm:dyn:fv:gmeanxy_sumfix")
     endif
  endif
