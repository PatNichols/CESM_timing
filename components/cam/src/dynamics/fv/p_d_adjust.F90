    ...
    if (jfirstxy==1) then
       ...
       if ( shr_reprosum_tolExceeded('p_d_adjust/South Pole', km*(ntotq+2), &
          ...
          if ( shr_reprosum_recompute ) then
             call t_startf("atm:dyn:fv:pdadj_sumfix")
             ...
             enddo
             deallocate( pole_tmp )
             call t_stopf("atm:dyn:fv:pdadj_sumfix")
          endif
       endif
