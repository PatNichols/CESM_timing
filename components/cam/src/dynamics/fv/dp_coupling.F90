...
!-----------------------------------------------------------------------
    ...
    call t_startf('atm:dyn:fv:d2a3dikj')
    ...
    call t_stopf('atm:dyn:fv:d2a3dikj')

    if ( do_circulation_diags ) then
       call t_startf('atm:dyn:fv:DP_CPLN_ctem')
       call ctem_diags( u3, v3, omgaxy, ptxy(:,jfirstxy:jlastxy,:), tracer(:,jfirstxy:jlastxy,:,1), &
                         psxy, pexy, grid)
       call t_stopf('atm:dyn:fv:DP_CPLN_ctem')
    endif
    if (use_gw_front .or. use_gw_front_igw) then
       call t_startf('atm:dyn:fv:DP_CPLN_gw_sources')
       call gws_src_fnct (u3,v3,ptxy,  tracer(:,jfirstxy:jlastxy,:,1), pexy, grid, frontgf, frontga)
       call t_stopf('atm:dyn:fv:DP_CPLN_gw_sources')
    end if
...
!dir$ concurrent
       ...
       call t_startf('atm:dyn:fv:block_to_chunk')
       call transpose_block_to_chunk(tsize, bbuffer, cbuffer)
       call t_stopf('atm:dyn:fv:block_to_chunk')
...
!
    call t_startf('atm:dyn:fv:derived_fields')
...
! Compute energy and water integrals of input state
    ...
    call t_stopf('atm:dyn:fv:derived_fields')
...
!dir$ concurrent
          ...
          call t_startf('atm:dyn:fv:chunk_to_block')
          call transpose_chunk_to_block(tsize, cbuffer, bbuffer)
          call t_stopf('atm:dyn:fv:chunk_to_block')
...
! -------------------------------------------------------------------------
       ...
       call t_barrierf('atm:dyn:fv:sync_uv3s_update', grid%commxy)
       call t_startf('atm:dyn:fv:uv3s_update')
       if (iam .lt. grid%npes_xy) then
          call uv3s_update( grid, dudtxy, u3sxy, dvdtxy, v3sxy, dt5 )
       end if  ! (iam .lt. grid%npes_xy)
       call t_stopf('atm:dyn:fv:uv3s_update')
...
! -------------------------------------------------------------------------
    call t_barrierf('atm:dyn:fv:sync_p_d_adjust', grid%commxy)
    call t_startf('atm:dyn:fv:p_d_adjust')
    if (iam .lt. grid%npes_xy) then
       call p_d_adjust(grid, tracer, dummy_pelnxy, pkxy, pkzxy, zvir,  cappa, &
                       delpxy, ptxy, pexy, psxy, ptop)
    end if  ! (iam .lt. grid%npes_xy)
    call t_stopf('atm:dyn:fv:p_d_adjust')
