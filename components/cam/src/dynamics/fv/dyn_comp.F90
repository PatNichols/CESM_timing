...
! Compute globally integrated Total Energy (te0)
         ...
         call t_startf('atm:dyn:fv:benergy')
!
...
!
         ...
         call t_stopf('atm:dyn:fv:benergy')
...
! (prime candidates: uc, vc, delpf)
   ...
   call t_startf('atm:dyn:fv:dyn_run_alloc')
...
! Set tracer limits to be supplied to trac2d (needed even without tracer decomposition)
   ...
   call t_stopf('atm:dyn:fv:dyn_run_alloc')
...
!
     ...
     call t_startf('atm:dyn:fv:xy_to_yz')
...
! Send tracers to auxiliary processes when decomposing
     ...
     call t_stopf('atm:dyn:fv:xy_to_yz')
...
!
     do 2000 n=1, n2
        ...
        if( ntotq > 0 ) then
           ...
           call t_startf('atm:dyn:fv:small_ts_init')
...
! Initialize total mass fluxes: (mfx, mfy)
           ...
           call t_stopf('atm:dyn:fv:small_ts_init')
...
! Call the Lagrangian dynamical core using small tme step
           ...
           call t_startf('atm:dyn:fv:cd_core')
           ...
           call t_stopf('atm:dyn:fv:cd_core')
...
!pw           call t_barrief('sync_vdot_gradp', grid%commdyn)
              call t_startf('atm:dyn:fv:vdot_gradp')
              if (grid%iam .lt. grid%npes_xy) then
                 call compute_vdot_gradp( grid, dt, dt/dt, cx_om, cy_om, pexy_om, omgaxy )
              endif  !  (grid%iam .lt. grid%npes_xy)
              call t_stopf('atm:dyn:fv:vdot_gradp')
...
! mass fluxes
           ...
           call t_startf('atm:dyn:fv:trac2d')
...
!   prior to n=n2 call to trac2d
           ...
           call t_stopf('atm:dyn:fv:trac2d')
...
!
           ...
           call t_startf('atm:dyn:fv:yz_to_xy_psuv')
...
! Transpose v
           ...
           call t_stopf('atm:dyn:fv:yz_to_xy_psuv')

           call t_startf('atm:dyn:fv:yz_to_xy_q')
...
!
           ...
           call t_stopf('atm:dyn:fv:yz_to_xy_q')
...
!
        ...
        call t_startf('atm:dyn:fv:te_map')
...
!$omp parallel do private(i,j,k)
        ...
        call t_stopf('atm:dyn:fv:te_map')
...
3000 continue  !    do 3000 iv = 1, nv - vertical re-mapping sub-cycle loop
  ...
  call t_startf('atm:dyn:fv:dyn_run_dealloc')
  ...
  call t_stopf('atm:dyn:fv:dyn_run_dealloc')
