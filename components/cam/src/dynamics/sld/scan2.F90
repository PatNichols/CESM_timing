...
!-----------------------------------------------------------------------
!
   call t_startf('atm:dyn:sld:grcalc_init')
grdpss(:,:)       = nan
grpss(:,:)        = nan
grpls(:,:)        = nan
grpms(:,:)        = nan
...
grqma(:,:,:) = nan
grqla(:,:,:) = nan
   call t_stopf('atm:dyn:sld:grcalc_init')

   call t_startf('atm:dyn:sld:grcalc')
...
!$OMP PARALLEL DO PRIVATE (LAT, J)
   ...
   call t_stopf('atm:dyn:sld:grcalc')

   call t_startf('atm:dyn:sld:spegrd_alloc')
#if ( defined SPMD )
   nlon_fft_in = 2*maxm
...
!
   ...
   call t_stopf('atm:dyn:sld:spegrd_alloc')

   call t_startf('atm:dyn:sld:spegrd_bft')
!$OMP PARALLEL DO PRIVATE (LAT, IROW)
   ...
   call t_stopf('atm:dyn:sld:spegrd_bft')

   call t_startf('atm:dyn:sld:spegrd_ift')
   call spegrd_ift ( nlon_fft_in, nlon_fft_out, fftbuf_in, fftbuf_out )
   call t_stopf('atm:dyn:sld:spegrd_ift')
                   
   call t_startf('atm:dyn:sld:spegrd_aft')
!$OMP PARALLEL DO PRIVATE (LAT, J)
...
!
   call t_startf('atm:dyn:sld:scan2_single')
   tmassf = 0._r8
   do lat=1,plat
...
!
   ...
   call t_stopf('atm:dyn:sld:scan2_single')

   call t_startf('atm:dyn:sld:tfilt_massfix')
...
!$OMP PARALLEL DO PRIVATE (LAT,J)
   ...
   call t_stopf ('atm:dyn:sld:tfilt_massfix')
!
! Dump Initial file
!
   dtime = get_step_size()
