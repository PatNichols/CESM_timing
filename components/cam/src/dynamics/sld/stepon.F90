...
!-----------------------------------------------------------------------
   ...
   !
   call t_startf('atm:dyn:sld:stepon_startup')
   ...
   call t_stopf('atm:dyn:sld:stepon_startup')
...
!
   ...
   !
   call t_startf('atm:dyn:sld:diag_dynvar_ic')
   call diag_dynvar_ic (phis, ps(:,:,n3), t3(:,:,:,n3), u3(:,:,:,n3), &
                        v3(:,:,:,n3), q3(:,:,:,:,n3) )
   call t_stopf('atm:dyn:sld:diag_dynvar_ic')
   !
   !----------------------------------------------------------
   ! PHYS_RUN Call the Physics package
   !----------------------------------------------------------
   !
   call t_startf('atm:dyn:sld:d_p_coupling')
   call d_p_coupling (ps(:,:,n3), t3(:,:,:,n3), u3(:,:,:,n3), &
                      v3(:,:,:,n3), q3(:,:,:,:,n3), &
                      omga, phis, phys_state, phys_tend,  pbuf2d)
   call t_stopf('atm:dyn:sld:d_p_coupling')
...
!
  ...
  type(dyn_export_t) :: dyn_out                      ! included for compatibility
   ...
   call t_startf('atm:dyn:sld:p_d_coupling')
   call p_d_coupling (phys_state, phys_tend, t2, fu, fv, flx_net,q3(:,:,:,:,n3))
   call t_stopf('atm:dyn:sld:p_d_coupling')
...
!
   ...
   call t_startf('atm:dyn:sld:dynpkg')
   call dynpkg(t2      ,fu      ,fv      ,etamid  , &
                cwava   ,detam   ,flx_net , ztodt  ,adv_state)
   call t_stopf('atm:dyn:sld:dynpkg')
