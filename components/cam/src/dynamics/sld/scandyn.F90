...
!-----------------------------------------------------------------------
!
  call t_startf('atm:dyn:sld:slt_run_setup')
  call slt_run_setup( adv_state )
  call t_stopf('atm:dyn:sld:slt_run_setup')

  call t_startf('atm:dyn:sld:linemsdyn')
!$OMP PARALLEL DO PRIVATE (LAT)
  ...
  call t_stopf('atm:dyn:sld:linemsdyn')

  call t_startf('atm:dyn:sld:sltrun')
  call slt_run( ztodt   ,detam   ,cwava   ,etamid  ,          &
                grlps1  ,grlps2  ,grt1    ,grt2    ,grq1    , &
                grq2    ,grfu1   ,grfu2   ,grfv1   ,grfv2   , &
                grfu    ,grfv    ,lnpssld ,prhssld ,adv_state )
  call t_stopf('atm:dyn:sld:sltrun')
!
!
  return
