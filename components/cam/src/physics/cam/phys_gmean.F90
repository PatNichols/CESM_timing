...
!-----------------------------------------------------------------------
!
      call t_startf ('atm:phys:cam:gmean_fixed_repro')
      call gmean_fixed_repro(arr, arr_gmean, rel_diff, nflds)
      call t_stopf ('atm:phys:cam:gmean_fixed_repro')
         ...
         if ( shr_reprosum_recompute ) then
            do ifld=1,nflds
               if ( rel_diff(1,ifld) > shr_reprosum_reldiffmax ) then
                  call t_startf ('atm:phys:cam:gmean_float_repro')
                  call gmean_float_repro(arr(:,:,ifld), arr_gmean(ifld), 1)
                  call t_stopf ('atm:phys:cam:gmean_float_repro')
               endif
            enddo
         endif
      endif
