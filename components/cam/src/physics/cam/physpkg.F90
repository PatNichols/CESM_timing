...
subroutine phys_run1(phys_state, ztodt, phys_tend, pbuf2d,  cam_in, cam_out)
    ...
    call t_startf('atm:phys:cam:physpkg_st1')
    nstep = get_nstep()
    ...
    call t_startf('atm:phys:cam:chk_en_gmean')
    call check_energy_gmean(phys_state, pbuf2d, ztodt, nstep)
    call t_stopf('atm:phys:cam:chk_en_gmean')

    call t_stopf('atm:phys:cam:physpkg_st1')

    if ( adiabatic .or. ideal_phys )then
       call t_startf('atm:phys:cam:bc_physics')
       call phys_run1_adiabatic_or_ideal(ztodt, phys_state, phys_tend,  pbuf2d)
       call t_stopf('atm:phys:cam:bc_physics')
    else
       call t_startf('atm:phys:cam:physpkg_st1')
...
!$OMP PARALLEL DO PRIVATE (C, phys_buffer_chunk)
       do c=begchunk, endchunk
          ...
          call t_startf('atm:phys:cam:diag_physvar_ic')
          call diag_physvar_ic ( c,  phys_buffer_chunk, cam_out(c), cam_in(c) )
          call t_stopf('atm:phys:cam:diag_physvar_ic')
...
!$OMP PARALLEL DO PRIVATE (C, FLX_HEAT)
    do c=begchunk, endchunk
       ...
       if ( ideal_phys )then
          call t_startf('atm:phys:cam:tphysidl')
          call tphysidl(ztodt, phys_state(c), phys_tend(c))
          call t_stopf('atm:phys:cam:tphysidl')
       end if
...
subroutine phys_run2(phys_state, ztodt, phys_tend, pbuf2d,  cam_out, &
    ...
    call t_startf('atm:phys:cam:lightning_no_prod')
    call lightning_no_prod( phys_state, pbuf2d,  cam_in )
    call t_stopf('atm:phys:cam:lightning_no_prod')

    call t_barrierf('sync_ac_physics', mpicom)
    call t_startf('atm:phys:cam:ac_physics')
    call t_adj_detailf(+1)
...
!$OMP PARALLEL DO PRIVATE (C, NCOL, phys_buffer_chunk)
    ...
    do c=begchunk,endchunk
       ...
       !
       call t_startf('atm:phys:cam:diag_surf')
       call diag_surf(cam_in(c), cam_out(c), phys_state(c), phys_buffer_chunk)
       call t_stopf('atm:phys:cam:diag_surf')
    ...
    call t_adj_detailf(-1)
    call t_stopf('atm:phys:cam:ac_physics')

#ifdef TRACER_CHECK
    call gmean_mass ('after tphysac FV:WET)', phys_state)
#endif

    call t_startf('atm:phys:cam:carma_accumulate_stats')
    call carma_accumulate_stats()
    call t_stopf('atm:phys:cam:carma_accumulate_stats')

    call t_startf('atm:phys:cam:physpkg_st2')
    call pbuf_deallocate(pbuf2d, 'physpkg')

    call pbuf_update_tim_idx()
    call diag_deallocate()
    call t_stopf('atm:phys:cam:physpkg_st2')
...
subroutine tphysac (ztodt,   cam_in,  &
    ...
    call t_startf('atm:phys:cam:tphysac_init')
    ! Associate pointers with physics buffer fields
    ...
    call t_stopf('atm:phys:cam:tphysac_init')
    !===================================================
    ! Source/sink terms for advected tracers.
    !===================================================
    call t_startf('atm:phys:cam:adv_tracer_src_snk')
    ! Test tracers
    ...
    call t_stopf('atm:phys:cam:adv_tracer_src_snk')
    ...
    call t_startf('atm:phys:cam:vertical_diffusion_tend')
   ...
   !------------------------------------------
    ...
    call t_stopf('atm:phys:cam:vertical_diffusion_tend')

    !===================================================
    ! Rayleigh friction calculation
    !===================================================
    call t_startf('atm:phys:cam:rayleigh_friction')
    call rayleigh_friction_tend( ztodt, state, ptend)
    call physics_update(state, ptend, ztodt, tend)
    call t_stopf('atm:phys:cam:rayleigh_friction')
    ...
    call t_startf('atm:phys:cam:aero_drydep')
    call aero_model_drydep( state, pbuf, obklen, surfric, cam_in, ztodt, cam_out, ptend )
    call physics_update(state, ptend, ztodt, tend)
    call t_stopf('atm:phys:cam:aero_drydep')
   ...
   if (carma_do_aerosol) then
     call t_startf('atm:phys:cam:carma_timestep_tend')
     ...
     call t_stopf('atm:phys:cam:carma_timestep_tend')
   end if
    ...
    ! Gravity wave drag
    !===================================================
    call t_startf('atm:phys:cam:gw_tend')
    ...
    call check_energy_chng(state, tend, "gwdrag", nstep, ztodt, zero, &
         zero, zero, flx_heat)
    call t_stopf('atm:phys:cam:gw_tend')
    ...
    call t_startf('atm:phys:cam:iondrag')
    ...
    call t_stopf('atm:phys:cam:iondrag')
...
subroutine tphysbc (ztodt, state,  &
    ...
    call t_startf('atm:phys:cam:bc_init')
    ...
    call t_stopf('atm:phys:cam:bc_init')

    !===================================================
    ! Global mean total energy fixer
    !===================================================
    call t_startf('atm:phys:cam:energy_fixer')
    ...
    call t_stopf('atm:phys:cam:energy_fixer')
    !
    !===================================================
    ! Dry adjustment
    ! This code block is not a good example of interfacing a parameterization
    !===================================================
    call t_startf('atm:phys:cam:dry_adjustment')

    call dadadj_tend(ztodt, state, ptend)

    call physics_update(state, ptend, ztodt, tend)

    call t_stopf('atm:phys:cam:dry_adjustment')

    !===================================================
    ! Moist convection
    !===================================================
    call t_startf('atm:phys:cam:moist_convection')

    call t_startf('atm:phys:cam:convect_deep_tend')
    ...
    call t_stopf('atm:phys:cam:convect_deep_tend')
    ...
    !
    call t_startf('atm:phys:cam:convect_shallow_tend')

    call convect_shallow_tend (ztodt   , cmfmc, &
         dlf        , dlf2   ,  rliq   , rliq2, & 
         state      , ptend  ,  pbuf, cam_in)
    call t_stopf('atm:phys:cam:convect_shallow_tend')
    ...
    call t_stopf('atm:phys:cam:moist_convection')
    ...
    if( microp_scheme == 'RK' ) then
       ...
       !===================================================
       call t_startf('atm:phys:cam:rk_stratiform_tend')
       ...
       call t_stopf('atm:phys:cam:rk_stratiform_tend')
    ...
    elseif( microp_scheme == 'MG' ) then
       ...
       do macmic_it = 1, cld_macmic_num_steps
          ...
          if (micro_do_icesupersat) then 
            ...
            ! Aerosol Activation
            !===================================================
            call t_startf('atm:phys:cam:microp_aero_run')
            call microp_aero_run(state, ptend, cld_macmic_ztodt, pbuf)
            call t_stopf('atm:phys:cam:microp_aero_run')
          ...
          call t_startf('atm:phys:cam:macrop_tend')
          ...
          call t_stopf('atm:phys:cam:macrop_tend')
          ...
          call t_startf('atm:phys:cam:microp_tend')
          ...
          call t_stopf('atm:phys:cam:microp_tend')
    ...
    if ( .not. deep_scheme_does_scav_trans() ) then
       ...
       call t_startf('atm:phys:cam:bc_aerosols')
       ...
       if (carma_do_wetdep) then
          ...
          call t_startf('atm:phys:cam:carma_wetdep_tend')
          call carma_wetdep_tend(state, ptend, ztodt, pbuf, dlf, cam_out)
          call physics_update(state, ptend, ztodt, tend)
          call t_stopf('atm:phys:cam:carma_wetdep_tend')
       end if

       call t_startf('atm:phys:cam:convect_deep_tend2')
       call convect_deep_tend_2( state,   ptend,  ztodt,  pbuf ) 
       call physics_update(state, ptend, ztodt, tend)
       call t_stopf('atm:phys:cam:convect_deep_tend2')

       ! check tracer integrals
       call check_tracers_chng(state, tracerint, "cmfmca", nstep, ztodt,  zero_tracers)

       call t_stopf('atm:phys:cam:bc_aerosols')
    ...
    call t_startf('atm:phys:cam:bc_history_write')
    call diag_phys_writeout(state, cam_out%psl)
    call diag_conv(state, ztodt, pbuf)

    call t_stopf('atm:phys:cam:bc_history_write')

    !===================================================
    ! Write cloud diagnostics on history file
    !===================================================

    call t_startf('atm:phys:cam:bc_cld_diag_history_write')

    call cloud_diagnostics_calc(state, pbuf)

    call t_stopf('atm:phys:cam:bc_cld_diag_history_write')

    !===================================================
    ! Radiation computations
    !===================================================
    call t_startf('atm:phys:cam:radiation')
    ...
    call t_stopf('atm:phys:cam:radiation')

    ! Diagnose the location of the tropopause and its location to the history file(s).
    call t_startf('atm:phys:cam:tropopause')
    call tropopause_output(state)
    call t_stopf('atm:phys:cam:tropopause')

    ! Save atmospheric fields to force surface models
    call t_startf('atm:phys:cam:cam_export')
    call cam_export (state,cam_out,pbuf)
    call t_stopf('atm:phys:cam:cam_export')

    ! Write export state to history file
    call t_startf('atm:phys:cam:diag_export')
    call diag_export(cam_out)
    call t_stopf('atm:phys:cam:diag_export')
...
!-----------------------------------------------------------------------------------
  ...
  if (do_waccm_ions) then
     ...
     call t_startf('atm:phys:cam:efield')
     call get_efield
     call t_stopf('atm:phys:cam:efield')
  endif
