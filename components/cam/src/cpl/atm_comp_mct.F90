...
!================================================================================
 ...
 subroutine atm_run_mct( EClock, cdata_a, x2a_a, a2x_a)
    ...
    call t_startf('atm:cpl:CAM_import')
    call atm_import( x2a_a%rattr, cam_in )
    call t_stopf('atm:cpl:CAM_import')
    ...
    do while (.not. dosend)
       ...
       call t_startf('atm:cpl:CAM_run2')
       call cam_run2( cam_out, cam_in )
       call t_stopf('atm:cpl:CAM_run2')

       call t_startf('atm:cpl:CAM_run3')
       call cam_run3( cam_out )
       call t_stopf('atm:cpl:CAM_run3')
       
       call t_startf('atm:cpl:CAM_run4')
       call cam_run4( cam_out, cam_in, rstwr, nlend, &
            yr_spec=yr_sync, mon_spec=mon_sync, day_spec=day_sync, sec_spec=tod_sync)
       call t_stopf('atm:cpl:CAM_run4')
       
       ! Advance cam time step 
       
       call t_startf('atm:cpl:CAM_adv_timestep')
       call advance_timestep()
       call t_stopf('atm:cpl:CAM_adv_timestep')
       
       ! Run cam radiation/clouds (run1)
          
       call t_startf('atm:cpl:CAM_run1')
       call cam_run1 ( cam_in, cam_out ) 
       call t_stopf('atm:cpl:CAM_run1')
       
       ! Map output from cam to mct data structures
       
       call t_startf('atm:cpl:CAM_export')
       call atm_export( cam_out, a2x_a%rattr )
       call t_stopf('atm:cpl:CAM_export')
