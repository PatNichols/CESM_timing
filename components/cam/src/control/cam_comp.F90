...
!-----------------------------------------------------------------------
   ...
   !----------------------------------------------------------
   !
   call t_barrierf ('atm:con:sync_phys_run1', mpicom)
   call t_startf ('atm:con:phys_run1')
   call phys_run1(phys_state, dtime_phys, phys_tend, pbuf2d,  cam_in, cam_out)
   call t_stopf  ('atm:con:phys_run1')
...
!-----------------------------------------------------------------------
   ...
   !
   call t_barrierf ('cam:sync_phys_run2', mpicom)
   call t_startf('atm:con:phys_run2')
   call phys_run2(phys_state, dtime_phys, phys_tend, pbuf2d,  cam_out, cam_in )
   call t_stopf('atm:con:phys_run2')

   !
   ! Second phase of dynamics (at least couple from physics to dynamics)
   !
   call t_barrierf ('cam:sync_stepon_run2', mpicom)
   call t_startf('atm:con:stepon_run2')
   call stepon_run2( phys_state, phys_tend, dyn_in, dyn_out )

   call t_stopf('atm:con:stepon_run2')

   if (is_first_step() .or. is_first_restart_step()) then
      call t_startf('atm:con:run2_memusage')
      call t_stopf('atm:con:run2_memusage')
   end if
...
!-----------------------------------------------------------------------
   ...
   !
   call t_barrierf ('cam:sync_stepon_run3', mpicom)
   call t_startf('atm:con:stepon_run3')
   call stepon_run3( dtime_phys, cam_out, phys_state, dyn_in, dyn_out )

   call t_stopf('atm:con:stepon_run3')

   if (is_first_step() .or. is_first_restart_step()) then
      call t_startf('atm:con:run3_memusage')
      call t_stopf('atm:con:run3_memusage')
   end if
...
!-----------------------------------------------------------------------
   ...
   !----------------------------------------------------------
   !
   call t_barrierf ('cam:sync_wshist', mpicom)
   call t_startf ('atm:con:wshist')
   call wshist ()
   call t_stopf  ('atm:con:wshist')

   !
   ! Write restart files
   !
   if (rstwr) then
      call t_startf('atm:con:write_restart')
      ...
      call t_stopf('atm:con:write_restart')
   end if

   call t_startf('atm:con:run4_wrapup')
   call wrapup(rstwr, nlend)
   call t_stopf('atm:con:run4_wrapup')
