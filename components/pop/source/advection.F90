...
!
! !REVISION HISTORY:
!  SVN:$Id: advection.F90 70398 2015-04-28 20:21:55Z mlevy@ucar.edu $
...
!-----------------------------------------------------------------------
   ...
   call get_timer(timer_advu,'OCN:ADVECTION_MOMENTUM', nblocks_clinic, &
                                                   distrb_clinic%nprocs)
 
   if (all(tadvect_itype == tadvect_centered)) then
      call get_timer(timer_advt,'OCN:ADVECTION_TRACER_CENTERED', &
                                 nblocks_clinic, distrb_clinic%nprocs)
   else if (all(tadvect_itype == tadvect_upwind3)) then
      call get_timer(timer_advt,'OCN:ADVECTION_TRACER_UPWIND3', &
                                 nblocks_clinic, distrb_clinic%nprocs)
   else if (all(tadvect_itype == tadvect_lw_lim)) then
      call get_timer(timer_advt,'OCN:ADVECTION_TRACER_LW_LIM', &
                                 nblocks_clinic, distrb_clinic%nprocs)
   else
      call get_timer(timer_advt,'OCN:ADVECTION_TRACER', &
                                 nblocks_clinic, distrb_clinic%nprocs)
   endif
