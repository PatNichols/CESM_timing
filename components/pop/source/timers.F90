...
!
! !REVISION HISTORY:
!  SVN:$Id: timers.F90 75114 2015-11-17 03:34:40Z jet $
!
! !USES:
...
!-----------------------------------------------------------------------
   ...
   do n=1,max_timers
      all_timers(n)%name = 'ocn_unknown_timer_name'
...
!-----------------------------------------------------------------------
   ...
   if (all_timers(timer_id)%in_use) then
      ...
      all_timers(timer_id)%name = 'ocn:unknown_timer_name'
...
!-----------------------------------------------------------------------
   ...
   if (all_timers(timer_id)%in_use) then
      ...
      call exit_POP(sigAbort, &
                    'ocn:timer_start: attempt to start undefined timer')
   endif
