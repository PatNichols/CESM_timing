...
!****************************************************************************
  ...
  subroutine pcg_solver_standard_3d(nx,        ny,            &
    ...
    call t_startf("cism_gliss:pcg_precond_init")
    call setup_preconditioner_3d(nx,         ny,       &
                                 ...
                                 Adiagu,     Adiagv,   &
                                 Muu,        Mvv)
    call t_stopf("cism_gliss:pcg_precond_init")
    ...
    call t_startf("cism_gliss:pcg_halo_init")
    call staggered_parallel_halo(xu)
    call staggered_parallel_halo(xv)
    call t_stopf("cism_gliss:pcg_halo_init")

    ! Compute A*x (use z as a temp vector for A*x)

    call t_startf("cism_gliss:pcg_matmult_init")
    call matvec_multiply_structured_3d(nx,        ny,            &
                                       ...
                                       xu,        xv,            &
                                       zu,        zv)
    call t_stopf("cism_gliss:pcg_matmult_init")

    ! Compute the initial residual r(0) = b - Ax(0)
    ! This will be correct for locally owned vertices.

    call t_startf("cism_gliss:pcg_vecupdate_init")
    ru(:,:,:) = bu(:,:,:) - zu(:,:,:)
    rv(:,:,:) = bv(:,:,:) - zv(:,:,:)
    call t_stopf("cism_gliss:pcg_vecupdate_init")
    ...
    call t_startf("cism_gliss:pcg_dotprod")
    work0u(:,:,:) = bu(:,:,:)*bu(:,:,:)    ! terms of dot product (b, b)
    work0v(:,:,:) = bv(:,:,:)*bv(:,:,:)
    call t_stopf("cism_gliss:pcg_dotprod")

    ! find global sum of the squared L2 norm

    call t_startf("cism_gliss:pcg_glbsum_init")
    call global_sum_staggered(nx,     ny,     &
                              nz,     nhalo,  &
                              L2_rhs,         &
                              work0u, work0v)
    call t_stopf("cism_gliss:pcg_glbsum_init")
    ...
    iter_loop: do n = 1, maxiters
       ...
       call t_startf("cism_gliss:pcg_precond")
       ...
       call t_stopf("cism_gliss:pcg_precond")
       ...
       call t_startf("cism_gliss:pcg_glbsum_iter")
       call global_sum_staggered(nx,     ny,     &
                                 nz,     nhalo,  &
                                 eta1,           &
                                 work0u, work0v)
       call t_stopf("cism_gliss:pcg_glbsum_iter")
       ...
       call t_startf("cism_gliss:pcg_vecupdate")
       ...
       call t_stopf("cism_gliss:pcg_vecupdate")

       ! Halo update for d

       call t_startf("cism_gliss:pcg_halo_iter")
       call staggered_parallel_halo(du)
       call staggered_parallel_halo(dv)
       call t_stopf("cism_gliss:pcg_halo_iter")
  
       ! Compute q = A*d
       ! This is the one matvec multiply required for each iteration

       call t_startf("cism_gliss:pcg_matmult_iter")
       call matvec_multiply_structured_3d(nx,        ny,            &
                                          nz,        nhalo,         &
                                          indxA,     active_vertex, &
                                          Auu,       Auv,           &
                                          ...
                                          du,        dv,            &
                                          qu,        qv)
       call t_stopf("cism_gliss:pcg_matmult_iter")
...
!!          write(6,*) 'eta1, eta2, alpha:', eta1, eta2, alpha
       ...
       if (mod(n,solve_ncheck) == 0) then
          ...
          call t_startf("cism_gliss:pcg_halo_resid")
          call staggered_parallel_halo(xu)
          call staggered_parallel_halo(xv)
          call t_stopf("cism_gliss:pcg_halo_resid")

          ! Compute A*x (use z as a temp vector for A*x)
           
          call t_startf("cism_gliss:pcg_matmult_resid")
          call matvec_multiply_structured_3d(nx,        ny,            &
                                             nz,        nhalo,         &
                                             indxA,     active_vertex, &
                                             Auu,       Auv,           &
                                             ...
                                             xu,        xv,            &
                                             zu,        zv)
          call t_stopf("cism_gliss:pcg_matmult_resid")
          ...
          call t_startf("cism_gliss:pcg_glbsum_resid")
          call global_sum_staggered(nx,     ny,       &
                                    nz,     nhalo,    &
                                    L2_resid,         &
                                    work0u, work0v)
          call t_stopf("cism_gliss:pcg_glbsum_resid")
...
!****************************************************************************
  ...
  subroutine pcg_solver_chrongear_3d(nx,        ny,            &
    ...
    call t_startf("cism_gliss:pcg_precond_iter")
    ...
    call t_stopf("cism_gliss:pcg_precond_iter")
...
!!       write(6,*) 'rho_old, sigma, alpha:', rho_old, sigma, alpha
    ...
    call t_startf("cism:gliss:pcg_vecupdate")
    xu(:,:,:) = xu(:,:,:) + alpha*du(:,:,:)
    xv(:,:,:) = xv(:,:,:) + alpha*dv(:,:,:)

    ru(:,:,:) = ru(:,:,:) - alpha*qu(:,:,:)     ! q = A*d
    rv(:,:,:) = rv(:,:,:) - alpha*qv(:,:,:)
    call t_stopf("cism:gliss:pcg_vecupdate")
    ...
    iter_loop: do n = 2, maxiters  ! first iteration done above
       ...
       call t_startf("cism:gliss:pcg_precond_iter")
       ...
       call t_startf("cism:gliss:pcg_glbsum_iter")
       call global_sum_staggered(nx,     ny,     &
                                 nz,     nhalo,  &
                                 gsum,           &
                                 work2u, work2v)
       call t_stopf("cism:gliss:pcg_glbsum_iter")

       !---- Halo update for Az
       !---- This is the one halo update required per iteration

       call t_startf("cism:gliss:pcg_halo_iter")
       call staggered_parallel_halo(Azu)
       call staggered_parallel_halo(Azv)
       call t_stopf("cism:gliss:pcg_halo_iter")
...
!!          write(6,*) 'rho, sigma, alpha:', rho, sigma, alpha
       ...
       if (mod(n,solve_ncheck) == 0) then    ! use r = b - Ax
          ...
          call t_startf("cism:gliss:pcg_matmult_resid")
          call matvec_multiply_structured_3d(nx,        ny,            &
                                             nz,        nhalo,         &
                                             indxA,     active_vertex, &
                                             Auu,       Auv,           &
                                             ...
                                             xu,        xv,            &
                                             zu,        zv)
          call t_stopf("cism:gliss:pcg_matmult_resid")
          ...
          call t_startf("cism:gliss:pcg_glbsum_resid")
          call global_sum_staggered(nx,     ny,       &
                                    nz,     nhalo,    &
                                    rr,               &
                                    worku, workv)
          call t_stopf("cism:gliss:pcg_glbsum_resid")
...
!             print*, 'iter, L2_resid, error =', n, L2_resid, err
          ...
          call t_startf("cism:gliss:pcg_halo_resid")
          call staggered_parallel_halo(ru)
          call staggered_parallel_halo(rv)
          call t_stopf("cism:gliss:pcg_halo_resid")
...
!****************************************************************************
  ...
  subroutine pcg_solver_chrongear_2d(nx,        ny,            &
    ...
    call t_startf("cism:gliss:pcg_precond_init")
    call setup_preconditioner_2d(nx,         ny,       &
                                 precond,    indxA,    &
                                 Auu,        Avv,      &
                                 Adiagu,     Adiagv)
    call t_stopf("cism:gliss:pcg_precond_init")
    ...
    call t_startf("cism:gliss:pcg_glbsum_init")
    call global_sum_staggered(nx,     ny,     &
                              nhalo,  bb,     &
                              worku,  workv)
    call t_stopf("cism:gliss:pcg_glbsum_init")
    ...
    call t_startf("cism:gliss:pcg_halo_init")
    call staggered_parallel_halo(xu)
    call staggered_parallel_halo(xv)
    call t_stopf("cism:gliss:pcg_halo_init")

    !---- Compute A*x   (use z as a temp vector for A*x)

    call t_startf("cism:gliss:pcg_matmult_init")
    call matvec_multiply_structured_2d(nx,        ny,            &
                                       nhalo,                    &
                                       indxA,     active_vertex, &
                                       Auu,       Auv,           &
                                       ...
                                       xu,        xv,            &
                                       zu,        zv)
    call t_stopf("cism:gliss:pcg_matmult_init")
