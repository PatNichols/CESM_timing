...
!  use glimmer_to_dycore
  ...
  call t_startf('cism:driver:cism')
 
  ! initialise GLIDE
  call t_startf('cism:driver:initialization')
...
!  do_glide_init = (wd == DYCORE_GLIDE) .OR. (wd == DYCORE_BISICLES) .OR. (wd == DYCORE_ALBANYFELIX)
 ...
 ! fill dimension variables on output files
  ...
  call t_stopf('cism:driver:initialization')

  if (model%options%whichdycore == DYCORE_BISICLES) then
    call t_startf('cism:driver:initial_diag_var_solve')
    ...
    call t_stopf('cism:driver:initial_diag_var_solve')
  endif
  ...
  ! ------------- Calculate initial state and output it -----------------
    ...
    call t_startf('cism:driver:initial_write_diagnostics')
    call glide_write_diagnostics(model,        time,       &
                                 tstep_count = tstep_count)
    call t_stopf('cism:driver:initial_write_diagnostics')
  ...
  call t_startf('cism:driver:initial_io_writeall')                                                          
  ...
  call t_stopf('cism:driver:initial_io_writeall')
...
!  external_dycore_model_index = this_rank + 1
  ...
  if (time < model%numerics%tend) then
    do while(time + time_eps < model%numerics%tend)
      ...
      call t_startf('cism:driver:read_forcing')
      call glide_read_forcing(model, model)
      call t_stopf('cism:driver:read_forcing')
...
! print *,"external_dycore_type: ",model%options%external_dycore_type
      ...
      call t_startf('cism:driver:tstep')

      select case (model%options%whichdycore)
        case (DYCORE_GLIDE)

          call t_startf('cism:driver:glide_tstep_p1')
          call glide_tstep_p1(model,time)
          call t_stopf('cism:driver:glide_tstep_p1')

          call t_startf('cism:driver:glide_tstep_p2')
          call glide_tstep_p2(model)
          call t_stopf('cism:driver:glide_tstep_p2')

          call t_startf('cism:driver:glide_tstep_p3')
          call glide_tstep_p3(model)
          call t_stopf('cism:driver:glide_tstep_p3')
      ...
      call t_stopf('cism:driver:tstep')
      !endif

      ! write ice sheet diagnostics to log file at desired interval (model%numerics%dt_diag)

      call t_startf('cism:driver:write_diagnostics')
      call glide_write_diagnostics(model,        time,       &
                                  tstep_count = tstep_count)
      call t_stopf('cism:driver:write_diagnostics')
      ...
      call t_startf('cism:driver:set_forcing')
      call eismint_massbalance(model%eismint_climate,model,time)
      call eismint_surftemp(model%eismint_climate,model,time)
      call t_stopf('cism:driver:set_forcing')
  ...
  !    call t_stopf('cism:driver:read_forcing')
      ...
      call t_startf('cism:driver:io_writeall')
      call glide_io_writeall(model,model)
      call t_stopf('cism:driver:io_writeall')
      ...
      !call t_startf('cism:driver:glide_io_writeall')
      !call glide_io_writeall(model,model)
      !call t_stopf('cism:driver:glide_io_writeall')  
  endif    
...
subroutine cism_finalize_dycore(model)
  ...
  if (model%options%whichdycore == DYCORE_BISICLES) then
    call t_startf('cism:driver:finalize_external_dycore')
    call cism_finalize_external_dycore(model%options%external_dycore_type,model)
    call t_stopf('cism:driver:finalize_external_dycore')
  endif
